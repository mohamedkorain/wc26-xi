-- Final manual assist correction (FIFA free feed logged null assister for the
-- Ferran Torres 106' goal). Per user: Nico Williams assisted (+1 assist).
-- Source: current xi_json (Final squad), submitted before the Final deadline.
-- Idempotent: sets assists to target, adds only the delta to points.
set statement_timeout='5min';
create temporary table tmp_fin_asst(match_date date, name text, nation text, assists int) on commit drop;
insert into tmp_fin_asst values ('2026-07-19','WILLIAMS Nico','Spain',1);

create temporary table tmp_fin_asst_elig on commit drop as
...
with existing_add as (
  select s.entry_id, s.match_date,
    jsonb_object_agg(e.name, coalesce(s.breakdown->e.name,'{}'::jsonb) || jsonb_build_object('assists', e.target)) as patch,
    sum(e.target - coalesce((s.breakdown->e.name->>'assists')::int,0))::int as added
  from public.scores s
  join tmp_fin_asst_elig e on e.entry_id=s.entry_id and e.match_date=s.match_date
...
select e.entry_id, e.match_date, sum(e.target)::int, jsonb_object_agg(e.name, jsonb_build_object('assists', e.target))
from tmp_fin_asst_elig e
...
select a.name, a.nation, count(*) filter (where (b.value->>'assists') is not null) as owners_with_assist,
