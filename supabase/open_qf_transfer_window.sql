-- Open the Quarter-Final transfer window safely.
--
-- QF rule:
--   - The R16 scoring squad is already frozen in xi_json_r16 (done separately
--     on 2026-07-09 in supabase/freeze_xi_json_r16_2026_07_09.sql).
...
do $$
declare
  v_qf_deadline constant timestamptz := '2026-07-09 19:00:00+00';
...
begin
  ...
  if v_previous_window is distinct from v_qf_deadline then
    ...
    begin
      update public.entries
         set xi_json_r16 = coalesce(xi_json_r16, xi_json),
             transfers_used = 0
       where league_id = '11111111-1111-1111-1111-111111111111'
         and (xi_json_r16 is null or coalesce(transfers_used, 0) <> 0);
  ...
  update public.leagues
     set transfers_open_until = v_qf_deadline
   where id = '11111111-1111-1111-1111-111111111111';

  raise notice 'QF transfer window prepared. entries_reset=% previous_window=%',
  ...
  if TG_OP = 'INSERT' then
    ...
    if now() >= v_locked_at
      ...
      if NEW.xi_json_gw1 is not null
         ...
         or NEW.xi_json_r16 is not null then
...
create trigger validate_entry_lineup_write_trg
  before insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3, xi_json_r32, xi_json_r16 on public.entries
...
begin
  ...
  if NEW.user_id is distinct from OLD.user_id
     ...
     or NEW.xi_json_r16 is distinct from OLD.xi_json_r16 then
    raise exception 'Only transfer updates are allowed after lock';
  end if;
  ...
  -- Does not consume the QF 3-out/1-in move and cannot alter snapshots.
  with old_rows as (
  ...
  if NEW.transfers_used is distinct from OLD.transfers_used + 1
    ...
    raise exception 'QF transfer update must use exactly one incoming player';
  end if;
  ...
  if v_new_empty_slots <> v_old_empty_slots + 2 then
    raise exception 'QF transfer must add exactly two new empty starter slots';
  end if;
  ...
  if v_changed_slots <> 3 or v_emptied_slots <> 2 or v_refilled_slots <> 1 then
    raise exception 'QF transfer must be exactly 3 out, 1 in, and 2 new empty slots';
  end if;
  ...
  if v_real_slots <> v_old_real_slots - 2 then
    raise exception 'QF transfer must remove exactly two net real players';
  end if;
...
create trigger entries_xi_sync_trg
  after insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3, xi_json_r32, xi_json_r16 on public.entries
...
select
  ...
  count(*) filter (where e.xi_json_r16 is not null) as r16_snapshots,
