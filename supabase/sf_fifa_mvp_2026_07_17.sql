-- SF FIFA official Player of the Match (+1 mvp). Source: FIFA Michelob POTM.
-- Match 101 France 0-2 Spain -> Pedro Porro; Match 102 England 1-2 Argentina -> Lionel Messi.
-- Sourced from current xi_json (SF squad). Eligibility: submitted before SF deadline.
set statement_timeout='5min';
create temporary table tmp_sf_mvp(match_date date, name text, nation text) on commit drop;
insert into tmp_sf_mvp values
 ('2026-07-14','PORRO Pedro','Spain'),
 ('2026-07-15','MESSI Lionel','Argentina');

create temporary table tmp_sf_mvp_elig on commit drop as
...
with existing_add as (
  ...
  join tmp_sf_mvp_elig e on e.entry_id=s.entry_id and e.match_date=s.match_date
