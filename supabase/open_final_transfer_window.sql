-- Open the FINAL transfer window safely.
--
-- Final rule (identical mechanic to QF/SF):
--   - The SF scoring squad (current xi_json) is frozen into xi_json_sf by this
--     script so the SF round display/scoring stays stable while Final transfers
--     edit current xi_json. Earlier snapshots (r16/qf) remain immutable.
--   - Reset transfers_used to 0 for the Final window.
...
--
-- Rerunning after the league is already open to the Final deadline will not
-- reset transfer counters or overwrite xi_json_sf.
...
alter table public.entries
  add column if not exists xi_json_sf jsonb;
...
do $$
declare
  v_final_deadline constant timestamptz := '2026-07-19 19:00:00+00';
...
begin
  ...
  if v_previous_window is distinct from v_final_deadline then
    ...
    begin
      update public.entries
         set xi_json_sf = coalesce(xi_json_sf, xi_json),
             transfers_used = 0
       where league_id = '11111111-1111-1111-1111-111111111111'
         and (xi_json_sf is null or coalesce(transfers_used, 0) <> 0);
  ...
  update public.leagues
     set transfers_open_until = v_final_deadline
   where id = '11111111-1111-1111-1111-111111111111';

  raise notice 'Final transfer window prepared. entries_reset=% previous_window=%',
...
begin
  ...
  if TG_OP = 'INSERT' or now() < v_locked_at then
    ...
    if NEW.xi_json_sf is not null then perform public.validate_entry_xi_json(NEW.xi_json_sf); end if;
  end if;
  ...
  if TG_OP = 'INSERT' then
    ...
    if now() >= v_locked_at
      ...
      if NEW.xi_json_gw1 is not null
         ...
         or NEW.xi_json_qf is not null
         or NEW.xi_json_sf is not null then
...
create trigger validate_entry_lineup_write_trg
  before insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3, xi_json_r32, xi_json_r16, xi_json_qf, xi_json_sf on public.entries
...
begin
  ...
  if NEW.xi_json is not distinct from OLD.xi_json
     ...
     and NEW.xi_json_sf is not distinct from OLD.xi_json_sf
  ...
  if NEW.user_id is distinct from OLD.user_id
     ...
     or NEW.xi_json_qf is distinct from OLD.xi_json_qf
     or NEW.xi_json_sf is distinct from OLD.xi_json_sf then
    raise exception 'Only transfer updates are allowed after lock';
  end if;
...
create trigger entries_xi_sync_trg
  after insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3, xi_json_r32, xi_json_r16, xi_json_qf, xi_json_sf on public.entries
...
cross join lateral jsonb_array_elements(
  ...
  || coalesce(e.xi_json_sf, '[]'::jsonb)
) as x(player)
where nullif(x.player->>'nation', '') is not null;

select
  l.transfers_open_until,
  count(*) filter (where e.xi_json_sf is not null) as sf_snapshots,
  count(*) filter (where e.xi_json_sf is distinct from e.xi_json) as sf_differs_from_current,
