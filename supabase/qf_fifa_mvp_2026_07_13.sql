-- QF FIFA official Player of the Match (+1 mvp). Source: FIFA Michelob POTM.
set statement_timeout='5min';
create temporary table tmp_qf_mvp(match_date date, name text, nation text) on commit drop;
insert into tmp_qf_mvp values
 ('2026-07-09','MBAPPE Kylian','France'),
 ('2026-07-10','YAMAL Lamine','Spain'),
 ('2026-07-11','BELLINGHAM Jude','England'),
 ('2026-07-11','ALVAREZ Julian','Argentina');

create temporary table tmp_qf_mvp_elig on commit drop as
...
with existing_add as (
  ...
  join tmp_qf_mvp_elig e on e.entry_id=s.entry_id and e.match_date=s.match_date
...
join public.player_pool pp on pp.name=b.key and pp.nation=m.nation and b.key=m.name
where (b.value->>'mvp') is not null
group by m.name, m.nation order by m.nation;
