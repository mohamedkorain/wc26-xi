-- Open the R32 transfer window safely.
--
-- What this does:
--   1) Adds an MD3 squad snapshot column.
--   2) Freezes every existing entry's current XI into xi_json_gw3 BEFORE
--      reopening transfers. That XI continues scoring all MD3 fixtures.
--   3) Resets transfers_used to 0 as the R32 per-window counter.
--   4) Reopens transfers until the R32 deadline:
--        2026-06-28 19:00 UTC = 22:00 Cairo.
...
--   6) Rebuilds entry_nations from xi_json + xi_json_gw1 + xi_json_gw2
--      + xi_json_gw3 so score-day still finds entries whose MD3 players
--      were later transferred out for R32.
--
-- Important: the backfill/reset block only runs if the league is not already
-- opened to the R32 deadline, so rerunning this script will not erase R32
-- transfer counts after users start making moves.

set statement_timeout = '5min';

alter table public.entries
  add column if not exists xi_json_gw3 jsonb;

do $$
declare
  v_r32_deadline constant timestamptz := '2026-06-28 19:00:00+00';
...
begin
  ...
  if v_previous_window is distinct from v_r32_deadline then
    if exists (
    ...
    begin
      update public.entries
         set xi_json_gw3 = coalesce(xi_json_gw3, xi_json),
             transfers_used = 0
  ...
  update public.leagues
     set transfers_open_until = v_r32_deadline
   where id = '11111111-1111-1111-1111-111111111111';

  raise notice 'R32 transfer window prepared. entries_reset=% previous_window=%',
  ...
  if TG_OP = 'INSERT' then
    ...
    if now() >= v_locked_at
      ...
      if NEW.xi_json_gw1 is not null
         ...
         or NEW.xi_json_gw3 is not null then
...
create trigger validate_entry_lineup_write_trg
  before insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3 on public.entries
...
begin
  ...
  if NEW.user_id is distinct from OLD.user_id
     ...
     or NEW.xi_json_gw3 is distinct from OLD.xi_json_gw3 then
    raise exception 'Only transfer updates are allowed after lock';
  end if;
...
create trigger entries_xi_sync_trg
  after insert or update of xi_json, xi_json_gw1, xi_json_gw2, xi_json_gw3 on public.entries
...
select
  ...
  count(*) filter (where e.xi_json_gw3 is not null) as gw3_snapshots,
